...
|---|---|---|
| **Google Programmable Search (Custom Search JSON API)** — используется как основной | Официальный, стабильный, поддерживает `gl`/`hl` (страна/язык) напрямую, не требует антибот-мер | 100 запросов/день бесплатно, далее платно; это отдельный индекс CSE, а не тот же ranking, что видит обычный пользователь в браузере; не различает Desktop/Mobile SERP |
| **Управляемая браузерная автоматизация** (`BrowserAutomationSearchProvider`, stub) | Может дать "живую" SERP с реальным device-специфичным рендерингом | Хрупкий (верстка Google меняется), вопрос соответствия ToS Google, риск CAPTCHA — при её появлении worker **обязан остановиться**, а не пытаться обойти |

Для MVP включён только первый вариант. Второй оставлен как явно
промаркированный экспериментальный stub с описанием рисков — сознательно
не реализован "из коробки".
