...
## 7. Обнаруженные и исправленные проблемы после реального тестирования

### 7.1 GTM/трекеры не находились ни разу — `waitUntil: "networkidle"`

**Симптом**: анализ любого реального сайта с аналитикой/рекламой всегда
возвращал пустой результат (0 GTM, 0 tracking ID, 0 внешних доменов).

**Причина**: `page.goto()` ждал состояния `"networkidle"` (сеть должна
"затихнуть" на 500мс). Сайты с аналитикой/рекламой почти никогда не
затихают — трекеры шлют периодические beacon-запросы бесконечно. В
результате `page.goto` стабильно упирался в таймаут (20с), анализ падал
со статусом `TIMEOUT` и пустыми находками — на любом сайте, где GTM
действительно есть, просто по факту того, что на нём есть GTM.

**Исправление**: `waitUntil: "domcontentloaded"` (наступает быстро и
предсказуемо) + фиксированная пауза `POST_LOAD_SETTLE_MS` (4 секунды)
после загрузки, чтобы асинхронные теги успели инициализироваться и
сделать свои первые запросы — которые мы и так ловим через `page.route()`
независимо от общего состояния сети страницы.

### 7.2 Резолвинг конечного домена по ссылкам (`redirectResolver.ts`)

По запросу: агент теперь дополнительно собирает все внешние `<a href>`
ссылки на странице (до 15 штук) и для каждой проходит по цепочке HTTP-
редиректов (HEAD/GET, ручной `manual` redirect, SSRF-проверка на каждом
шаге), сохраняя финальный домен назначения в `ExternalResource.finalDomain`
(resourceType `LINK`). Это отвечает на вопрос "ссылка выглядит как домен
A, но реально ведёт на домен B" — важно для сайтов, которые маскируют
переходы через редирект-сервисы.

**Ограничение**: работает только для HTTP-редиректов (301/302/303/307/308).
Переходы через JavaScript (`window.location = ...`) без HTTP-редиректа не
резолвятся — это потребовало бы полноценной навигации Playwright по каждой
ссылке, что для простого резолвинга избыточно по времени.

### 7.3 urlscan.io как второй источник для "Расширить сеть"

По запросу: раньше "Расширить сеть" использовала только текстовый поиск
(Serper) по литеральным GTM/tracking ID — если сайт нигде не упоминает
свой ID текстом, он не находился. Добавлен второй, независимый источник:
`discovery/urlscanProvider.ts` ищет через публичную базу urlscan.io
**другие страницы, которые загружали ресурс с того же внешнего домена**
(общий трекер/API/CDN) — это находит сайты с общей инфраструктурой, даже
если нигде текстом её не упоминают. Работает без API-ключа (с урезанной
квотой по IP) или с `URLSCAN_API_KEY` для более высокого лимита.

### 7.4 Firestore composite indexes — не создаются автоматически

Все компаунд-запросы (`.where().where()` на разных полях) в Firestore
требуют явно созданного составного индекса — иначе падают с
`FAILED_PRECONDITION`. Это касается: очереди job'ов у worker'а
(`status`+`createdAt`) и rate limiter'а на Vercel (`userId`+`status`,
`userId`+`createdAt`). Список — в `firestore.indexes.json`; создаются
либо вручную через Firebase Console → Firestore → Indexes, либo (в
production) через `firebase deploy --only firestore:indexes`.

### 7.5 Firestore Security Rules не применяются сами по себе

Файл `firestore.rules` в репозитории — это только исходник. Чтобы он
реально заработал, его нужно вручную вставить в Firebase Console →
Firestore Database → Rules → Publish (или задеплоить через
`firebase deploy --only firestore:rules`). Без этого действующие правила
— это то, что было выбрано при создании базы (обычно "запретить всё"),
и любая прямая запись с клиента (например, создание проекта) будет
падать с `Missing or insufficient permissions`.

## 8. Локальный запуск (кратко)
